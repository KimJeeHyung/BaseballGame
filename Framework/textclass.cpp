...
bool TextClass::SetFPS(int fps, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence1, fpsString, 20, 40, red, green, blue, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetScreenSize(int width, int height, ID3D11DeviceContext* deviceContext)
{
	...
	strcpy_s(sizeString, "ScreenWidth = ");
	strcat_s(sizeString, tempString);
	strcat_s(sizeString, ", ScreenHeight = ");

	_itoa_s(height, tempString, 10);
	strcat_s(sizeString, tempString);

	result = UpdateSentence(m_sentence4, sizeString, 20, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetNumberOfObjects(int objects, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char sizeString[32];
	bool result;
	...
	strcpy_s(sizeString, "Number of Objects = ");
	strcat_s(sizeString, tempString);

	result = UpdateSentence(m_sentence5, sizeString, 20, 80, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetNumberOfPolygons(int, ID3D11DeviceContext*)
{
	return false;
}
...
bool TextClass::SetIntersection(bool intersection, ID3D11DeviceContext* deviceContext)
{
	...
	if (intersection)
	{
		strcpy_s(intersectionString, "Hit: Yes");
		result = UpdateSentence(m_sentence3, intersectionString, 20, 120, 0.0f, 1.0f, 0.0f, deviceContext);
	}
	else
	{
		strcpy_s(intersectionString, "Hit: No");
		result = UpdateSentence(m_sentence3, intersectionString, 20, 120, 1.0f, 0.0f, 0.0f, deviceContext);
	}
